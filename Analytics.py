...
class CreateAnalyticsBar():
    def generateBar():
            ...
            data = pd.read_sql_query('''SELECT date, MIN(temp), MAX(temp), MIN(humid),
                MAX(humid) FROM pidata GROUP BY date''', conn)
            conn.close()
            data = pd.melt(data, id_vars="date", var_name="Attributes", value_name="Value")

            sns.catplot(x='date', y='Value', hue='Attributes', data=data, kind='bar')
